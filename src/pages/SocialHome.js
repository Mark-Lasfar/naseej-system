...
const StoryCircle = ({ story, onClick }) => {
    ...
    return (
        ...
        >
            <div className={`relative w-16 h-16 sm:w-20 sm:h-20 rounded-full p-[2px] ${hasUnseen ? 'bg-gradient-to-tr from-pink-500 via-red-500 to-yellow-500' : 'bg-gray-300'}`}>
                <div className="w-full h-full rounded-full bg-white p-[2px]">
                    <div className="w-full h-full rounded-full bg-gradient-to-r from-blue-600 to-purple-600 flex items-center justify-center text-white text-lg sm:text-2xl overflow-hidden">
            ...
            <span className="text-xs mt-1 text-center line-clamp-1 max-w-[60px] sm:max-w-[80px]">{story.user?.storeId?.name || story.user?.username}</span>
        </motion.div>
    );
};

// Main SocialHome Component - محدث للتجاوب الكامل
        ...
        <div className="max-w-full md:max-w-2xl lg:max-w-3xl xl:max-w-4xl mx-auto px-3 sm:px-4 md:px-6 py-4 md:py-6" ref={feedRef}>
            ...
            {user && (
                ...
                >
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
                        <div>
                            <h2 className="text-lg md:text-2xl font-bold">Welcome back, {user.username}! 👋</h2>
                            <p className="text-white/80 text-xs md:text-sm mt-1">Discover what's happening in your community</p>
                        </div>
                        <div className="flex gap-2 md:gap-3">
                            <div className="text-center">
                                <div className="text-lg md:text-2xl font-bold">{stats.totalLikes}</div>
                                <div className="text-[10px] md:text-xs text-white/80">Likes</div>
                            </div>
                            <div className="text-center">
                                <div className="text-lg md:text-2xl font-bold">{stats.totalComments}</div>
                                <div className="text-[10px] md:text-xs text-white/80">Comments</div>
                            </div>
                            <div className="text-center">
                                <div className="text-lg md:text-2xl font-bold">{stats.activeDays}</div>
                                <div className="text-[10px] md:text-xs text-white/80">Active</div>
                            </div>
                        </div>
                    </div>
                </motion.div>
                    ...
                    <div className="flex gap-3 md:gap-4 overflow-x-auto pb-2 scrollbar-thin">
                        {user && (
                            <div className="flex flex-col items-center cursor-pointer flex-shrink-0" onClick={() => setShowCreateModal(true)}>
                                <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-full bg-gray-200 flex items-center justify-center border-2 border-dashed border-blue-500">
            ...
            {user && (
                <motion.div
                    initial={{ opacity: 0, y: 20 }}
                    animate={{ opacity: 1, y: 0 }}
                    className="bg-white rounded-xl md:rounded-2xl shadow-md p-3 md:p-4 mb-4 md:mb-6"
                >
            ...
            {/* Feed Type Selector - متجاوب */}
            <div className="flex gap-1 md:gap-2 mb-4 md:mb-6 border-b pb-2 overflow-x-auto">
                <button
                    onClick={() => setFeedType('for_you')}
                    className={`px-3 md:px-5 py-1.5 md:py-2 rounded-full font-medium text-sm md:text-base transition whitespace-nowrap ${
                        feedType === 'for_you'
                            ? 'bg-gradient-to-r from-blue-600 to-purple-600 text-white shadow-md'
                            : 'text-gray-600 hover:bg-gray-100'
                    }`}
                >
                    <FaFire className="inline mr-1 md:mr-2 text-xs md:text-sm" /> For You
                </button>
                {user && (
                    <button
                    ...
                    >
                        <FaUserFriends className="inline mr-1 md:mr-2 text-xs md:text-sm" /> Following
                    </button>
                )}
                <button
                ...
                >
                    <FaRocket className="inline mr-1 md:mr-2 text-xs md:text-sm" /> Trending
                </button>
            </div>
